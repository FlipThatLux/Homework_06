...
def sort_items_by_type(path):                           # робить огляд файлів і папок, нормалізує імена та складає списки файлів на сортування
    
    for content in path.iterdir():
        if content.is_dir():
            if content.name in sorted_items.keys() and path == Path(sys.argv[1]):
                folders_to_not_touch.append(content)
                continue
            elif os.listdir(content) == []:
...
sorted_list_of_items = sort_items_by_type(path)
